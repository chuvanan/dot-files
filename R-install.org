...
#+BEGIN_SRC bash
./configure --prefix=/opt/R/3.6.3 --enable-R-shlib --with-blas --with-cairo --with-libtiff --enable-memory-profiling --without-recommended-packages --disable-java
make
sudo make install
#+END_SRC
...
#+BEGIN_SRC bash
sudo ln /opt/R/3.6.3/bin/R -s R
sudo ln /opt/R/3.6.3/bin/Rscript -s Rscript
#+END_SRC
